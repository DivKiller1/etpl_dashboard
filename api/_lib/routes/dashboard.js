...
router.get('/data', async (req, res) => {
    try {
        ...
        // Build match objects before parallel fetch
        const expenseMatch = { customerId: { $exists: true, $ne: null }, status: { $regex: '^paid$', $options: 'i' } };
        if (expRange)       expenseMatch.date = { $gte: formatDateStr(expRange.start), $lte: formatDateStr(expRange.end) };
        else if (cutoffStr) expenseMatch.date = { $gte: cutoffStr };
        ...
        if (cutoffStr) leafMatch.$or = [{ endDate: { $gte: cutoffStr } }, { startDate: { $gte: cutoffStr } }];

        // All queries in parallel — was 8 sequential awaits
        const [activeEmployees, allUsersForManagers, customerExpenses, rawPaidExpenses, attendances, sites, customers, leaves] =
            await Promise.all([
                User.find({ isDeleted: false, isSuspended: false, roleId: { $not: { $regex: /director/i } }, designation: { $not: { $regex: /director/i } } })
                    .select('_id fullName lastName roleId designation employeeId dateOfJoining managerDetails managerId baseLocation isSuspended').lean(),
                User.find({}).select('_id fullName lastName email').lean(),
                Expense.aggregate([
                    { $match: expenseMatch },
                    { $addFields: { custIdObj: { $convert: { input: '$customerId', to: 'objectId', onError: null, onNull: null } }, siteIdObj: { $convert: { input: '$siteId', to: 'objectId', onError: null, onNull: null } } } },
                    { $group: { _id: { customerId: '$custIdObj', siteId: '$siteIdObj', type: { $ifNull: ['$type', 'Other'] } }, total: { $sum: '$amount' } } },
                    { $lookup: { from: 'customers', localField: '_id.customerId', foreignField: '_id', as: 'customer' } },
                    { $unwind: '$customer' },
                    { $group: { _id: { customerId: '$_id.customerId', siteId: '$_id.siteId' }, name: { $first: '$customer.name' }, siteTotal: { $sum: '$total' }, types: { $push: { type: '$_id.type', total: '$total' } } } },
                    { $addFields: { siteIdObj: '$_id.siteId' } },
                    { $lookup: { from: 'sites', localField: 'siteIdObj', foreignField: '_id', as: 'site' } },
                    { $unwind: { path: '$site', preserveNullAndEmptyArrays: true } },
                    { $group: { _id: '$_id.customerId', name: { $first: '$name' }, total: { $sum: '$siteTotal' }, sites: { $push: { siteId: '$_id.siteId', name: { $ifNull: ['$site.name', 'Unknown Site'] }, total: '$siteTotal' } }, allTypes: { $push: '$types' } } },
                    { $project: { _id: 0, customerId: '$_id', name: 1, total: 1, sites: 1, types: { $reduce: { input: '$allTypes', initialValue: [], in: { $concatArrays: ['$$value', '$$this'] } } } } }
                ]),
                Expense.find(expenseMatch).select('customerId siteId type amount date').lean(),
                Attendance.find(trendMatch).select('userId date siteId').lean(),
                Site.find({}).select('_id name customerId').lean(),
                Customer.find({}).select('_id name').lean(),
                Leave.find(leafMatch).select('userId startDate endDate status type').lean()
            ]);

        const activeCount = activeEmployees.length || 1;

        // Build lookup maps from parallel-fetched data
        const customerMap = {};
        ...
        activeEmployees.forEach(emp => {
            const name = emp.fullName ? emp.fullName + (emp.lastName ? ' ' + emp.lastName : '') : 'Unknown';
            userMap[emp._id.toString()]     = name;
            userRoleMap[emp._id.toString()] = emp.designation || emp.roleId || 'Staff';
        });

        // Manager name map — derived from allUsersForManagers (replaces second sequential User.find)
        const userFullNameMap = {};
        const userEmailFullNameMap = {};
        allUsersForManagers.forEach(u => {
            const name = ((u.fullName || '') + (u.lastName ? ' ' + u.lastName : '')).trim();
            if (name) {
                userFullNameMap[u._id.toString()] = name;
                if (u.email) userEmailFullNameMap[u.email.toLowerCase().trim()] = name;
            }
        });

        // paidExpenses via JS map — replaces second sequential Expense.aggregate with $lookup
        const paidExpenses = rawPaidExpenses.map(e => ({
            customerName: customerMap[e.customerId?.toString()] || 'Unknown',
            customerId:   e.customerId?.toString() || '',
            siteName:     siteMap[e.siteId?.toString()] || 'Office',
            date:         e.date,
            category:     normalizeExpenseType(e.type || 'Other'),
            amount:       e.amount
        }));
        ...
        const finalizedCustomerExpenses = customerExpenses.map(c => {
            ...
            return { ...c, types: Object.keys(typeMap).map(k => ({ type: k, total: typeMap[k] })) };
        });

        // Daily attendance flat list
        ...
        attendances.forEach(a => {
            ...
            dailyAttendances.push({
                ...
                siteId:       sId || 'office',
                siteName:     sId ? (siteMap[sId] || 'Unknown Site') : 'Office',
                customerName: sId ? (siteCustomerMap[sId] || 'ETPL') : 'ETPL'
            });
        });
        ...
        attendances.forEach(a => {
            ...
            if (!uId) return;
            if (!weeklyData[week]) weeklyData[week] = { presentDays: {}, leaveDays: {}, leaveTypes: {}, range: getWeekRange(date) };
        ...
        leaves.forEach(l => {
            ...
            while (current <= end) {
                ...
                if (!weeklyData[week]) weeklyData[week] = { presentDays: {}, leaveDays: {}, leaveTypes: {}, range: getWeekRange(current) };
        ...
        let weeklyTrends = Object.keys(weeklyData).sort().map(week => {
            ...
            let sumPresentPct = 0, sumLeavePct = 0, sumAbsentPct = 0;
            ...
            const employeeDetails = activeEmployees.map(emp => {
                ...
                let ePresent = 0, eLeave = 0;
                const dailyStatus = {};
                weekDays.forEach(day => {
                    if      (pDays.has(day)) { dailyStatus[day] = 'Present';  ePresent++; }
                    else if (lDays.has(day)) { dailyStatus[day] = 'On Leave'; eLeave++;   }
                    else                      { dailyStatus[day] = 'Absent';               }
                });
                let pPct = Math.min((ePresent / 6) * 100, 100);
                let lPct = Math.min((eLeave   / 6) * 100, 100 - pPct);
                let aPct = Math.max(100 - pPct - lPct, 0);
                sumPresentPct += pPct; sumLeavePct += lPct; sumAbsentPct += aPct;
                const empName = emp.fullName ? emp.fullName + (emp.lastName ? ' ' + emp.lastName : '') : 'Unknown';
            ...
            return { week, range: w.range, present: presentPct, onLeave: onLeavePct, absent: absentPct, leaveDistribution: w.leaveTypes || {}, totalEmployees: activeCount };
        });
        ...
        const activeEmployeesDetails = activeEmployees.map(emp => {
            const empName = emp.fullName ? emp.fullName + (emp.lastName ? ' ' + emp.lastName : '') : 'Unknown';
            let mName = 'N/A';
            if (emp.managerId && userFullNameMap[emp.managerId.toString()])                                          mName = userFullNameMap[emp.managerId.toString()];
            else if (emp.managerDetails?.email && userEmailFullNameMap[emp.managerDetails.email.toLowerCase().trim()]) mName = userEmailFullNameMap[emp.managerDetails.email.toLowerCase().trim()];
            else if (emp.managerDetails?.name)                                                                       mName = emp.managerDetails.name;
            return { _id: emp._id, employeeId: emp.employeeId || '', fullName: empName, designation: emp.designation || emp.roleId || 'Staff', dateOfJoining: emp.dateOfJoining || '', managerName: mName, baseLocation: emp.baseLocation || '', isSuspended: emp.isSuspended || false };
        });

        const approvedLeaves = leaves.map(l => {
            const start    = l.startDate || '';
            const end      = l.endDate   || start;
            const duration = (start && end) ? Math.max(1, Math.ceil((new Date(end) - new Date(start)) / 86400000) + 1) : 1;
            return { employeeName: userMap[l.userId?.toString()] || 'Unknown', startDate: start, endDate: end, duration, type: l.type || 'Other' };
        }).filter(l => l.startDate);

        res.json({ success: true, data: { availableWeeks: [...new Set(availableWeeks)].sort(), weeklyTrends, detailedAttendance, customerExpenses: finalizedCustomerExpenses, dailyAttendances, employees: activeEmployeesDetails, approvedLeaves, paidExpenses } });
    } catch (err) {
        console.error(err);
...
router.get('/director-data', async (req, res) => {
    try {
        ...
        // ── Round 1: 3 parallel fetches (was 5 sequential) ───────────────────
        const [allUsers, allSites, allCustomers] = await Promise.all([
            User.find({}).select('_id fullName lastName email roleId designation employeeId dateOfJoining managerDetails managerId baseLocation isSuspended isDeleted').lean(),
            Site.find({ isDeleted: false }).lean(),
            Customer.find({}).lean()
        ]);

        // Build user name maps (replaces second User.find)
        const userFullNameMap     = {};
        const userEmailFullNameMap = {};
        allUsers.forEach(u => {
            const name = ((u.fullName || '') + (u.lastName ? ' ' + u.lastName : '')).trim();
        ...
        const getEmpManagerName = (emp) => {
            if (emp.managerId && userFullNameMap[emp.managerId.toString()])                                             return userFullNameMap[emp.managerId.toString()];
            if (emp.managerDetails?.name)                                                                               return emp.managerDetails.name;
            if (emp.managerDetails?.email && userEmailFullNameMap[emp.managerDetails.email.toLowerCase().trim()])       return userEmailFullNameMap[emp.managerDetails.email.toLowerCase().trim()];
            return 'N/A';
        };

        // Derive active employees in JS (replaces separate DB query)
        let activeEmployees = allUsers.filter(u => !u.isDeleted && !u.isSuspended && !/director/i.test(u.roleId || '') && !/director/i.test(u.designation || ''));
        if (dirStateFilter) {
        ...
        const activeEmpMap         = new Map(activeEmployees.map(e => [e._id.toString(), e]));
        const totalEmployees       = activeEmployees.length || 1;

        // Derive etplCustId in JS (replaces Customer.findOne)
        const etplCust   = allCustomers.find(c => /etpl/i.test(c.name));
        const etplCustId = etplCust ? etplCust._id.toString() : null;

        // Filter sites
        let sites = etplCustId ? allSites.filter(s => s.customerId && s.customerId.toString() !== etplCustId) : [...allSites];
        if (dirCustFilter) sites = sites.filter(s => s.customerId && s.customerId.toString() === dirCustFilter);
        const totalSites = sites.length;

        const statusBreakdown = {};
        sites.forEach(s => { const st = s.status || 'Planned'; statusBreakdown[st] = (statusBreakdown[st] || 0) + 1; });

        // Filter customers (replaces Customer.find)
        const customersList = etplCustId ? allCustomers.filter(c => c._id.toString() !== etplCustId) : [...allCustomers];
        const customerListMap = new Map(customersList.map(c => [c._id.toString(), c]));
        ...
        // ── Round 2: 3 parallel fetches (was 9 sequential) ──────────────────
        const [attendancesToday, thirtyDayAttendances, allLeavesBroad] = await Promise.all([
            Attendance.find({ date: today }).lean(),
            Attendance.find({ date: { $gte: thirtyDaysAgo } }).select('userId siteId date').lean(),
            Leave.find({ status: 'Approved', startDate: { $lte: today }, endDate: { $gte: thirtyDaysAgo } }).lean()
        ]);

        // Derive today-specific data from full result sets in JS
        const shiftStartUnix = Math.floor(new Date(`${today}T09:45:00+05:30`).getTime() / 1000);
        const lateEmployeesToday  = attendancesToday.filter(a => { const uId = a.userId?.toString(); return uId && activeEmployeeIdsSet.has(uId) && a.checkInTime > shiftStartUnix; });
        const travelAttendances   = attendancesToday.filter(a => { const uId = a.userId?.toString(); return uId && activeEmployeeIdsSet.has(uId) && /travel|tour|visit/i.test(a.checkInRemark || ''); });
        const leavesTodayAll      = allLeavesBroad.filter(l => l.endDate >= today);
        const leavesTodayActive   = leavesTodayAll.filter(l => l.userId && activeEmployeeIdsSet.has(l.userId.toString()));

        // Derive distinct-ID sets from thirtyDayAttendances (replaces 3 .distinct() queries)
        const activeSiteIdsStr  = new Set(thirtyDayAttendances.map(a => a.siteId ? a.siteId.toString() : '').filter(Boolean));
        const recentSiteIdsStr  = new Set(thirtyDayAttendances.filter(a => a.date >= sevenDaysAgo).map(a => a.siteId ? a.siteId.toString() : '').filter(Boolean));
        const recentlyActiveIdsStr = new Set(thirtyDayAttendances.filter(a => a.date >= fiveDaysAgo).map(a => a.userId ? a.userId.toString() : '').filter(Boolean));

        // Present / leave counts
        const presentTodayActive = [];
        attendancesToday.forEach(att => { const uId = att.userId?.toString(); if (uId && activeEmployeeIdsSet.has(uId)) presentTodayActive.push(att.userId); });
        ...
        const travelingCount           = travelAttendances.length;
        ...
        const activeSiteEngMap          = new Map(activeSiteEngineers.map(e => [e._id.toString(), e]));
        const totalSiteEngineersCount   = activeSiteEngineers.length;
        const deployedSiteEngineersCount = presentTodayActive.filter(id => activeSiteEngineerIdsSet.has(id.toString())).length;
        ...
        // Sites without engineers / at risk
        ...
        const sitesWithoutEngineerAlert = sitesWithoutEngineers.map(s => {
            const cust = customerListMap.get(s.customerId?.toString());
            return {
                _id: s._id,
                siteId: s.siteId,
                name: s.name,
        ...
        attendancesToday.forEach(att => {
            ...
            if (uId && activeEmployeeIdsSet.has(uId)) {
                const emp = activeEmpMap.get(uId);
        ...
        const detailOnLeave = leavesTodayActive.map(l => {
            const emp = activeEmpMap.get(l.userId.toString());
            return {
        ...
        const detailTotalSites = sites.map(s => {
            const cust = customerListMap.get(s.customerId?.toString());
            return { siteId: s.siteId || 'N/A', name: s.name, projectName: cust ? cust.name : 'Unknown Project', status: s.status || 'Planned', state: s.state || 'N/A', district: s.district || 'N/A' };
        });

        const detailLateEmployees = [];
        lateEmployeesToday.forEach(att => {
            const emp = activeEmpMap.get(att.userId?.toString());
            if (emp) {
                const sId = att.siteId?.toString();
                const siteName = sId ? (siteMap[sId] || 'Office') : 'Office';
                const checkInTimeFormatted = att.checkInTime 
        ...
        attendancesToday.forEach(att => {
            ...
            if (uId && activeSiteEngineerIdsSet.has(uId)) {
                const emp = activeSiteEngMap.get(uId);
                const sId = att.siteId?.toString();
                const siteName = sId ? (siteMap[sId] || 'Office') : 'Office';
                const customerName = sId ? (siteCustomerMap[sId] || 'ETPL') : 'ETPL';
        ...
        // 11. Trends (Last 30 Days) — reuse thirtyDayAttendances + allLeavesBroad (no extra queries)
        const startDateStr = getPastDateStr(29);
        const endDateStr   = getPastDateStr(0);

        const attendancesByDate = {};
        thirtyDayAttendances.forEach(a => {
            if (!a.date) return;
            const sId = a.siteId?.toString();
